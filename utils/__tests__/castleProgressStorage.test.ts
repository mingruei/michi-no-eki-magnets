...
jest.mock('expo-file-system', () => {
  ...
  class MockFile {
    ...
    uri = 'file:///mock/castle-progress-v1.json';
...
describe('castleProgressStorage', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('migrates legacy AsyncStorage progress to file storage', async () => {
    mockedGetItem.mockResolvedValue(
      JSON.stringify({
        2: createProgressEntry({ goshuin: true }),
      }),
    );

    const map = await loadProgressMap();
    expect(map[2]?.goshuin).toBe(true);
    expect(mockedRemoveItem).toHaveBeenCalledWith('castle-progress-v1');
  });
